...
**编译方式**：用户**以网页端编译器提交**；**本机亦已装 TeX Live 2026**（2026-09-24，`D:\Software\texlive\2026`），骨架可**本地编译自检**——实测 `mcm-2027-summary.tex` **2 页 / 71 KB / 零 Overfull**；官方模板依赖 `newtx`＋`fontaxes`，缺则报 `newtxtext.sty` not found。**但页数、体积这类量仍以用户提交的那一次编译为准**——本地与网页端 TeX 版本不同、换页点可能不同，**不得**用本地结果代填。凡声称编译结果须附真实命令与输出，**不得**再以"本机无法编译"为由跳过验证。若网页端报错，**把报错原文整段贴回来**即可定位（常见的是宏包缺失或 `\Problem`／`\Team` 未替换）。`[社区]`（**操作纪律**／本项目环境约束；官方无对应原文）
...
9. **声称"编译通过"而其实没编译。** 本机现已能编译（TeX Live 2026），所以"编译通过"必须附真实命令与输出；未编译却说通过，会让队伍带着未知错误交稿。`[社区]`（**操作纪律**）
...
11. **在 `.tex` 里写英文以外的注释（尤其中文）。** 官方模板与修正副本都是**纯 ASCII**，故本项建议保持注释用英文、与官方原件同一口径。**实测**（TeX Live 2026／pdfLaTeX，2026-09-24）：**注释**里混中文与全角标点**不会**报错（退出码 0、PDF 正常产出，原「会报编码类错误」的说法**已被实测推翻**）；**正文**里混中文才**会**直接失败（`! LaTeX Error: Unicode character 中 (U+4E2D)`）。骨架的注释一律用英文；队伍自己的批注也建议先用英文写，交稿前清干净。`[社区]`（**操作纪律**；若网页端报出编码／字符类错误，先查源码里有没有非 ASCII 字符——**尤其查正文**，并把报错原文贴回来）
